...
def retrive_physaddr_bits_num():
    ...
    dmi = dmi.split('\n')
    total_sz = 0
    p = re.compile(r"^\s*Size: (\d+) GB")
    for l in dmi:
        m = re.match(p, l.strip())
        if m:
            print("Size: {} GB".format(m.group(1)))
            total_sz += int(m.group(1))
    
    print("total phy mem size: {} GB, bit count: {}".format(total_sz, 30 + int(math.log(total_sz, 2))))
    return 30 + int(math.log(total_sz, 2))
